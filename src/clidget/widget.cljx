...
(defmacro for [bindings & [body]]
  (let [parsed-bindings (clj/for [[variable binding] (partition 2 bindings)]
                          (if-not (#{:when :let :while} variable)
                            (let [surrogate-var (gensym "surrogate")]
                              {:binding-pair [surrogate-var binding
                                              :let [variable surrogate-var]]
                               :let-vars [[variable surrogate-var]]
                               :surrogate-vars [surrogate-var]})
                            {:binding-pair [variable binding]
                             :let-vars (when (= :let variable)
                                         (partition-all 2 binding))}))
        
        binding-pairs (mapcat :binding-pair parsed-bindings)
        surrogate-vars (mapcat :surrogate-vars parsed-bindings)
        let-vars (mapcat :let-vars parsed-bindings)
        widget-type (gensym "clidget-for")]

    `(let [loop-iters# (clj/for ~(vec binding-pairs)
                         [~@surrogate-vars])]
       (loop [[[~@surrogate-vars :as vars#] & more#] loop-iters#
              res-var# []]
         (if vars#
           (let [~@(apply concat let-vars)]
             (recur more# (conj res-var# ~body)))
           res-var#)))))
